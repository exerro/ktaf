...
internal fun UINode.computeWidthInternal(widthAllocated: Float?) {
    ...
    // * this' width ktaf.core.minus its padding
    // * width allocated to this, ktaf.core.minus this' margin and padding
    ...
    val contentWidth by when (val l = layout) {
        ...
        // compute the width of all children
        //  width allocated to child is normal width allocation, ktaf.core.minus the spacing between elements, and finally
...
internal fun UINode.positionChildrenInternal(heightAllocated: Float?) {
    ...
    // * this' height ktaf.core.minus its padding
    // * height allocated to this, ktaf.core.minus this' margin and padding
    ...
    val contentHeight by when (val l = layout) {
        ...
        is GridLayout -> {
            val ha = heightAllocatedInternal ?.let { h -> (h - (l.vertical - 1) * l.spacing.y) / l.vertical }
    ...
    when (val l = layout) {
        ...
        is FreeLayout -> {
            fun evalw(l: LayoutLineValue?) = l ?.let { l.x + (computedWidth - padding.width) * l.y / 100f }
            fun evalh(l: LayoutLineValue?) = l ?.let { l.x + (computedHeight - padding.height) * l.y / 100f }
